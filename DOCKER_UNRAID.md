...
- `CHROME_CDP` (예: `http://100.87.250.109:9222`)
- `DATABASE`
...
- `CHROME_CDP` (예: `http://100.87.250.109:9222` 또는 `ws://100.87.250.109:9222/devtools/browser/<id>`)
...
- `chrome_cdp`는 Playwright가 연결할 원격 브라우저 CDP 엔드포인트(CDP/WS)이며, 브라우저 서버에서 접근 가능해야 합니다.
- 호환성을 위해 기존 `chromedriver` 값도 fallback으로 읽지만, 신규 설정은 `chrome_cdp` 사용을 권장합니다.
